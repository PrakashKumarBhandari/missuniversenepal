...
add_image_size('video_thumbnail', 255,380, true);
...
function required_custom_post_type_and_taxonomies(){
	...
	register_post_type('contestants', array(
		'labels' => array('name' => 'Contestants'),
		'public' => true,
		'menu_position'=> 24,
		'supports' => array('title'),
	...
	register_post_type('avatars', array(
		'labels' => array('name' => 'Avatars'),
		'public' => true,
		'menu_position'=> 24,
		'supports' => array('title'),
...
function my_header_add_to_cart_fragment_change( $fragments ) {
 
    ob_start();
    $count = WC()->cart->cart_contents_count;
    ?>
	<a class="choose_my_cart" href="<?php echo WC()->cart->get_cart_url(); ?>">
...
<?php
    ...
    $fragments['a.choose_my_cart'] = ob_get_clean();
     
    return $fragments;
}
add_filter( 'woocommerce_add_to_cart_fragments', 'my_header_add_to_cart_fragment_change' );
...
function add_class_to_thumbs( $html, $attachment_id ) {
   ...
   if ( get_post_thumbnail_id() !== intval( $attachment_id ) ) {
       $html  = '<div class="single-zoom-thumb"> <ul class="s-tab-zoom owl-carousel single-product-active" id="gallery_01">';
       // I am wraping then product thumbnail into my custom div class-name 'productImg'
       $html .= sprintf( '<li><a><img src="'.$src[0].'" alt="%s" class="wp-post-image" /></a></li>', esc_url( wc_placeholder_img_src( 'woocommerce_single' ) ), esc_html__( 'Awaiting product image', 'woocommerce' ) );
       $html .= '</ul></div>';
   }
   return $html;
}
